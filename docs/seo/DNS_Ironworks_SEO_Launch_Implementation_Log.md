...
## September 22, 2026 — Follow-up: keep discovery and buying answers current

Further review found two maintenance gaps and a thin initial contact page. Fixed locally:

- `/sitemap.xml` now reads the same current public service and product collections as the pages. Additions and removals appear without a rebuild, explicit empty collections remain empty, duplicate paths are removed, and only canonical public URLs are emitted. A content-store failure returns a temporary 503 with Retry-After instead of quietly publishing a stale sitemap. The generated static sitemap remains available for static-only development; production must deploy the API.
- Product specifications and regular/XL comparisons now resolve from the current saved descriptions in both browser and initial HTML. Explicit supported dimension wording is extracted; missing/unrecognized dimensions are omitted. Removed pre-made comparison products stop appearing in the guide's links. This supersedes the earlier instruction to maintain hard-coded guide dimensions manually. Editorial buying advice still lives in product-guides.ts.
- The contact page's initial HTML now contains visible phone, SMS, email, and Facebook links. ContactPage structured data identifies the business, and homepage/service/contact markup shares a stable business ID. Contact descriptions now agree before and after JavaScript loads.
- Route verification now reads the served sitemap by default, rather than only validating the build-time file.

Validation: frontend/API TypeScript and builds pass; 54 served sitemap routes and canonicals pass; seven services, 43 products, 17 service answers, and nine product answers pass. The new local-only `verify-content-updates.mjs` passes changed dimensions, cross-product comparisons, new/removed product URLs, cleared collections, and contact HTML tests, restoring its fixture in a finally block. Browser verification confirms current 16×32/16×18 regular specifications and 22×32/22×20 XL comparison text, with no horizontal overflow. The existing nonfatal tooltip sourcemap warning remains.
